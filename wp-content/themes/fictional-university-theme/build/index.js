...
class Like {
  ...
  events() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(".like-box").on("click", this.ourClickDispacher.bind(this));
  }
  ...
  createNote(e) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      success: response => {
        ...
        jquery__WEBPACK_IMPORTED_MODULE_0___default()( /* HTML */`<li data-id="${response.id}">
					<input
						readonly
						class="note-title-field"
						value="${response.title.raw}"
					/>
					<span class="edit-note"
						><i class="fa fa-pencil" area-hidden="true"></i> Edit</span
					>
					<span class="delete-note"
						><i class="fa fa-trash-o" area-hidden="true"></i> Delete</span
					>
...
class Search {
  ...
  events() {
    this.openButton.on("click", this.openOverlay.bind(this)); // The on method, changes the value of "this" keyword to point towards the element searched
    this.closeButton.on("click", this.closeOverlay.bind(this));
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on("keydown", this.keyPressDispatcher.bind(this)); // keyup only fires once, and keydown gets call again and again and again
      ...
      this.resultsDiv.html( /* HTML */`
					<div class="row">
						...
						<div class="one-third">
							...
							${results.programs.length ? '<ul class="link-list min-list">' : /* HTML */` <p>
										No programs match that search.
										<a href="${universityData.root_url}/programs"
											>View all programs</a
										>
								  </p>`}
							...
							${results.professors.length ? '<ul class="professor-cards">' : /* HTML */` <p>No professors match that search.</p>`}
							${results.professors.map(item => /* HTML */`
						...
						<div class="one-third">
							...
							${results.campuses.length ? '<ul class="link-list min-list">' : /* HTML */`<p>
										No campuses match that search.
										<a href="${universityData.root_url}/campuses"
											>View all campuses</a
										>
								  </p>`}
							...
							${results.events.length ? "" : /* HTML */`<p>
										No events match that search.
										<a href="${universityData.root_url}/events"
											>View all events</a
										>
								  </p>`}
							${results.events.map(item => /* HTML */`
										<div class="event-summary">
											...
											<div class="event-summary__content">
												...
												<p>
													${item.description}
													<a href="${item.permalink}" class="nu gray"
														>Learn more</a
													>
												</p>
											</div>
